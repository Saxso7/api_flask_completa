...
from app import app
from app.database.database import dietHigh_Ref, get_next_id_dietHigh, verify_firebase_token
from app.models.models import dietHighAPI, dietHigh_api, dietHigh_model

# Crear una instancia de dietHighAPI y api para las rutas de dieta Resistencia
dietHighAPI = dietHighAPI
api = dietHigh_api

# Definir el modelo de dieta Resistencia
diedietHigh_model = dietHigh_model
...
class GetDietRes(Resource):
    def get(self):
        ...
        if authenticated:
            try:
                # Cambia la colección a dietHigh_Ref para obtener dietas Resistencia desde la base de datos
                diet_collection = dietHigh_Ref.stream()
...
class DeleteDietRes(Resource):
    def delete(self, diet_id):
        ...
        diet_doc = dietHigh_Ref.document(diet_id).get()
        if diet_doc.exists:
            dietHigh_Ref.document(diet_id).delete()
...
class UpdateDietRes(Resource):
    ...
    def put(self, diet_id):
        ...
        if request.headers['Content-Type'] == 'application/json':
            try:
                user_doc = dietHigh_Ref.document(diet_id).get()
                if user_doc.exists:
                    descripcionDesayuno = request.json['descripcionDesayuno']
                    desayuno = request.json['desayuno']
                    descripcionDesayuno = request.json['descripcionDesayuno']
                    ...
                    dietHigh_Ref.document(diet_id).update(diet_data)
